...
def click_and_wait_contents(driver, element):
    ...
    This works in Blaise 5.2.5, but not in Blaise 5.5.8.
    """
    try:
...
def click_and_wait_ie(driver, element):
    """
    Same als click_and_wait_contents(), but with an extra wait for the splash screen to disappear. 
    This works on Blaise 5.5.8 in IE, but not on Chrome or Firefox. For Chrome and Firefox, use click_and_wait_splash()
    instead.
    """
    #print("waiting")
    try:
...
def click_and_wait_splash(driver, element):
    ...
    It also does not work for IE: the IE webdriver appears to be too slow to catch the splash screen in some
    circumstances. The result is that IE is still waiting for the splash screen to appear while it has already
    disappeared. Use click_and_wait_ie() instead.
    """ 
    try:
        element.click()
    except (
